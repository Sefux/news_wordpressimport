<?php
namespace Projektkater\NewsWordpressimport\Service\Migrate;

use TYPO3\CMS\Core\Utility\ExtensionManagementUtility;
use TYPO3\CMS\Core\Utility\GeneralUtility;

class WordpressPluginMigrate
{
    ...
    public function __construct()
    {
        ...
        $url = ExtensionManagementUtility::extPath('news_wordpressimport') . 'Resources/Private/Template/flexform.txt';
    ...
    protected function migrate(array $row)
    {
        ...
        if ($row['sys_language_uid'] > 0 && $row['l18n_parent'] > 0) {
            $parentRow = $this->getDatabaseConnection()->exec_SELECTgetSingleRow(
                'uid',
                'tt_content',
                'news_wordpressimport_new_id=' . $row['l18n_parent']);
            if (is_array($parentRow)) {
                $update['l18n_parent'] = $parentRow['uid'];
            }
        }

        $this->getDatabaseConnection()->exec_UPDATEquery('tt_content', 'uid=' . $row['news_wordpressimport_new_id'],
            $update);
    }
    ...
    protected function getSysCategoryIdList($idList)
    {
        ...
        $categoryRows = $this->getDatabaseConnection()->exec_SELECTgetRows(
            'uid,title,import_id',
            'sys_category',
            'deleted=0 AND import_source="WP_CATEGORY_IMPORT" AND import_id IN('
     ...
     * @return array
     */
    protected function getWordpressPluginRows()
    {
